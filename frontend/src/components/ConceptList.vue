...
const typeMeta = {
  method: { label: '研究方法', color: 'var(--vermilion)' },
  theory: { label: '理论基础', color: 'var(--violet)' },
  dataset: { label: '数据集', color: 'var(--emerald)' },
  finding: { label: '研究发现', color: 'var(--amber)' },
  tool: { label: '工具/系统', color: 'var(--cyan)' },
}
...
const groupedConcepts = computed(() => {
  ...
  return Object.entries(groups).map(([type, items]) => ({
    ...
    color: typeMeta[type]?.color || 'var(--text-muted)',
    items,
  }))
})
</script>
...
.cl-count { font-size: var(--text-xs); color: var(--text-muted); background: rgba(255,255,255,0.05); padding: 1px 8px; border-radius: 10px; }
...
.cl-search-input { width: 100%; padding: 6px 10px; border: 1px solid var(--border-subtle); border-radius: var(--radius-sm); background: rgba(255,255,255,0.04); color: var(--text-primary); font-size: var(--text-sm); font-family: inherit; outline: none; }
...
.cl-item:hover { background: rgba(255,255,255,0.04); color: var(--text-primary); }
